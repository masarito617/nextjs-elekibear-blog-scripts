 ...
 * 対象とする記事種別
 */
const TargetPostType = 'page';

/**
 * 固定記事ページ
 * @returns
 */
const PagePostPage: NextPage<PostBasePageProps> = (
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  // URLパス
  const postUrlPath = `/page/${encodeURI(slug).toLowerCase()}`;
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  return {
    paths: allPosts.posts.map((post) => `/page/${post.slug}`) || [],
    fallback: false,
  };
};
